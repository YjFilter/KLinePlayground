...
init_training_routes(active_trainings, user_manager)
...
def _compact_crypto_period_snapshot(snapshot):
    ...
    compact['kline_data'] = [
        {
            'time': bar.get('start_time') or bar.get('time') or bar.get('end_time') or bar.get('datetime'),
...
INDEX_ASSET_RE = re.compile(r'((?:src|href)=")((?:js|css)/[^"]+?\.(?:js|css))(\?v=[^"]*)?(")')


def _versioned_index_html():
    """返回给 js/css 引用按文件 mtime 追加 ?v= 的首页 HTML；读取失败返回 None。"""
    html_path = os.path.join(frontend_folder, 'index_enhanced.html')
    try:
    ...
    def _stamp(match):
        ...
        file_path = os.path.join(frontend_folder, rel_path.replace('/', os.sep))
        try:
            version = int(os.stat(file_path).st_mtime)
        except OSError:
            return match.group(0)
        return f'{match.group(1)}{rel_path}?v={version}{match.group(4)}'

    return INDEX_ASSET_RE.sub(_stamp, html)


@app.route('/')
def index():
    """提供前端入口页面；js/css 版本号按文件 mtime 自动生成，杜绝手改 ?v= 遗忘导致的旧缓存"""
    html = _versioned_index_html()
    if html is None:
...
def _start_crypto_training(*, user, mode, period, initial_capital, training_id, payload, max_training_days):
    ...
    training = {
        ...
        'maker_fee_rate': str(payload.get('maker_fee_rate', DEFAULT_CRYPTO_MAKER_FEE_RATE) or DEFAULT_CRYPTO_MAKER_FEE_RATE),
        'taker_fee_rate': str(payload.get('taker_fee_rate', DEFAULT_CRYPTO_TAKER_FEE_RATE) or DEFAULT_CRYPTO_TAKER_FEE_RATE),
        'crypto_session': session,
        'crypto_bundle': bundle,
